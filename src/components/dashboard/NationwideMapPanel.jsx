...
export default function NationwideMapPanel({ data, selectedSido, selectedDistrict, mapLevel, onSelectSido, onSelectDistrict, onShowNationwide }) {
  ...
  return <section className="panel nationwide-map-panel">
    <div className="map-panel-head"><div><span className="eyebrow">{isDistrictView ? 'DISTRICT POLICY MAP' : 'NATIONWIDE POLICY MAP'}</span><h2>{isDistrictView ? `${selected.name} 시군구 지도` : '전국 청년 주거 인프라 지도'}</h2></div><span className="prototype-badge"><span className="live-dot" /> INFRA-LENS</span></div>
