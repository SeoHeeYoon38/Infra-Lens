...
export default function CategoryBars({ region }) {
  ...
  return <section className="panel category-panel"><SectionTitle eyebrow="DECISION EVIDENCE" title="왜 이 지역을 우선 검토하나요?" meta="최근 6개월 누계" /><div className="decision-score"><strong>{risk}%</strong><span>편의점 집중도</span><em>{risk >= 65 ? '즉시 검토' : risk >= 50 ? '현장 확인' : '추적 관찰'}</em></div><div className="category-bars">{rows.map((row) => <div className="category-row" key={row.label}><div className="category-name"><i style={{ background: row.color }} />{row.label}</div><div className="category-track"><span style={{ width: `${(row.value / max) * 100}%`, background: row.color }} /></div><strong>{formatWon(row.value)}</strong></div>)}</div><div className="category-note"><span className="note-icon">→</span> 편의점 소비가 신선 유통을 대체할수록 회수·분리배출 거점 우선순위를 높입니다.</div></section>
}
