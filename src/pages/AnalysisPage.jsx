...
export default function AnalysisPage() {
  ...
  const loadReport = async (shouldPrint = false) => {
    if (!selectedRegion || reportLoading) return
    setReportLoading(true)
    if (shouldPrint) setPrintPending(true)
    try {
  ...
  const handleReport = () => loadReport(false)
  const handlePrint = () => {
    if (report) {
  ...
  return <main className="page-main analysis-main">
    ...
    <section className="action-panel panel"><div className="action-icon"><img src="/assets/infra-lens-dog-cutout.png" alt="Infra-Lens 분석 안내 강아지" /></div><div className="action-copy"><span className="eyebrow">NEXT POLICY MOVE</span><h2>{selectedRegion.district}를 스마트 회수 거점 후보로 검토하세요.</h2><p>리포트를 생성하면 취약도, 소비 근거, 1인가구 규모를 묶어 실행 순서까지 제안합니다.</p></div><div className="action-stats"><div><span>추천 반경</span><strong>500m</strong></div><div><span>검증 레이어</span><strong>3개</strong></div><button type="button" onClick={handlePrint} disabled={reportLoading}>{reportLoading ? 'PDF 준비 중…' : '인쇄 / PDF 저장'} <span aria-hidden="true">↓</span></button></div></section>
  </main>
}
